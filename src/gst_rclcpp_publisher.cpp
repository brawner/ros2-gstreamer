...
gst_rclcpp_publisher_class_init (GstRclcppPublisherClass * klass)
{
  ...
  for (auto encoding : EncodingConversions::supported_gst_encodings()) {
    ss << encoding << ", ";
  }
  ss << "}"
...
gst_rclcpp_publisher_show_frame (GstVideoSink * sink, GstBuffer * buf)
{
  ...
  GstPublisherNode::Publish(rclcpp_publisher->node.get(), buf, width, height);
...
void GstPublisherNode::Publish(
    GstPublisherNode* node, GstBuffer* image_buffer, size_t width, size_t height) {
